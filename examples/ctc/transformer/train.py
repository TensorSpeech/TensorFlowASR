...
def main(
    ...
    with strategy.scope():
        ...
        transformer.make(speech_featurizer.shape, batch_size=global_batch_size)
        if pretrained:
            transformer.load_weights(pretrained, by_name=file_util.is_hdf5_filepath(pretrained), skip_mismatch=True)
        optimizer = tf.keras.optimizers.Adam(
            TransformerSchedule(d_model=transformer.dmodel, **config.learning_config.learning_rate_config),
