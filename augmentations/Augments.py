...
class Noise(Augmentation):
  def __init__(self, snr_list: list, min_noises: int, max_noises: int, noise_dir: str):
    ...
    super(Noise, self).__init__(func=functools.partial(add_noise, snr_list=self.snr_list, min_noises=self.min_noises,
                                                       max_noises=self.max_noises, noises=self.noises), is_post=False)
...
class WhiteNoise(Augmentation):
  def __init__(self, snr_list: list):
    ...
    if not any([i == 0 for i in self.snr_list]):
      self.snr_list.append(0)
    super(WhiteNoise, self).__init__(func=functools.partial(add_white_noise, snr_list=self.snr_list), is_post=False)
...
class RealWorldNoise(Augmentation):
  def __init__(self, snr_list: list, min_noises: int, max_noises: int, noise_dir: str):
    ...
    self.max_noises = max_noises
    self.noises = glob.glob(os.path.join(noise_dir, "**", "*.wav"), recursive=True)
    super(RealWorldNoise, self).__init__(func=functools.partial(add_realworld_noise, snr_list=self.snr_list,
                                                                min_noises=self.min_noises, max_noises=self.max_noises,
                                                                noises=self.noises), is_post=False)
