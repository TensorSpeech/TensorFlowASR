...
class CtcLoss(BaseLoss):
    ...
    def call(self, y_true, y_pred):
        ...
        losses = classic_ctc_loss(labels=labels, logits=logits, label_length=label_length, logit_length=logit_length, blank_index=self.blank)
        return losses
...
class BaseCtcLossData(ABC):
    ...
    def hessian(self) -> tf.Tensor:
        ...
        mask = expand_many_dims(inputs=tf.linalg.band_part(tf.ones(shape=[self.max_logit_length] * 2, dtype=tf.bool), 0, -1), axes=[0, 2, 4])
...
def expand_many_dims(inputs: tf.Tensor, axes: List[int]) -> tf.Tensor:
...
def classic_ctc_loss(
    ...
    Args:
        labels:         tf.Tensor, shape = [batch, max_label_length],       dtype = tf.int32
        logits:         tf.Tensor, shape = [batch, max_length, num_tokens], dtype = tf.float32
        label_length:   tf.Tensor, shape = [batch],                         dtype = tf.int32
        logit_length:   tf.Tensor, shape = [batch],                         dtype = tf.int32
...
class ClassicCtcLossData(BaseCtcLossData):
    ...
    def gamma(self) -> tf.Tensor:
        ...
        mask = expand_many_dims(
            inputs=tf.linalg.band_part(tf.ones(shape=[self.max_logit_length_plus_one] * 2, dtype=tf.bool), 0, -1), axes=[0, 2, 3, 5, 6]
        )
