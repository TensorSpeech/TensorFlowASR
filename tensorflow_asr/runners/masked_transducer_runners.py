...
class MaskedTransducerTrainer(BaseTrainer):
    def __init__(self,
                 config: RunningConfig,
                 text_featurizer: TextFeaturizer,
                 strategy: tf.distribute.Strategy = None):
        self.text_featurizer = text_featurizer
        super(MaskedTransducerTrainer, self).__init__(config, strategy=strategy)
    ...
    def _train_step(self, batch):
        _, features, input_length, labels, label_length, prediction, prediction_length, mask = batch

        with tf.GradientTape() as tape:
            logits = self.model([features, input_length, prediction, prediction_length, mask], training=True)
            tape.watch(logits)
    ...
    def _eval_step(self, batch):
        ...
        logits = self.model([features, input_length, prediction, prediction_length, mask], training=False)
        eval_loss = rnnt_loss(
            logits=logits, labels=labels, label_length=label_length,
            logit_length=get_reduced_length(input_length, self.model.time_reduction_factor),
            blank=self.text_featurizer.blank
...
# TODO: Create MaskedTransducerGATrainer for working with gradient accumulation
